# Resource: awx_workflow_job_template_node_success_node_association
...
```hcl
resource "awx_workflow_job_template_node_success_node_association" "example" {
...
- `success_node_id` (Number, Required) Child object numeric ID.
...
- `success_node_id` (Number, Read-Only) Child object numeric ID.
## Import

```bash
terraform import awx_workflow_job_template_node_success_node_association.example <primary_id>:<related_id>
```
