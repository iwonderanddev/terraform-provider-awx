...
Manages AWX associations between `workflow_job_nodes` and `instance_groups` objects.
...
- `workflow_job_node_id` (Number, Read-Only) Parent object numeric ID.
- `instance_group_id` (Number, Read-Only) Child object numeric ID.
## Import

```bash
terraform import awx_workflow_job_node_instance_group_association.example <primary_id>:<related_id>
```
