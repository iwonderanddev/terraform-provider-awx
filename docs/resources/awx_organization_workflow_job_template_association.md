...
Manages AWX associations between `organizations` and `workflow_job_templates` objects.
...
- `workflow_job_template_id` (Number, Read-Only) Child object numeric ID.
## Import

```bash
terraform import awx_organization_workflow_job_template_association.example <primary_id>:<related_id>
```
