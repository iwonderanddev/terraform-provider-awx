...
Manages `workflow_job_template_node_label_association` relationships between `workflow_job_template_nodes` and `labels` objects.

Breaking change: use `workflow_job_template_node_id` and `label_id` instead of legacy `parent_id` and `child_id`.
...
```bash
terraform import awx_workflow_job_template_node_label_association.example 12:34
```
