...
- `auth_ldap_1_group_type_params` (Optional, Computed) Terraform object value. Key value parameters to send the chosen group type init method.
- `auth_ldap_1_organization_map` (Optional, Computed) Terraform object value. Mapping between organization admins/users and LDAP groups. This controls which users are placed into which organizations relative to their LDAP group memberships. Configuration details are available in the documentation.
...
- `auth_ldap_1_team_map` (Optional, Computed) Terraform object value. Mapping between team members (users) and LDAP groups. Configuration details are available in the documentation.
- `auth_ldap_1_user_attr_map` (Optional, Computed) Terraform object value. Mapping of LDAP user schema to API user attributes. The default setting is valid for ActiveDirectory but users with other LDAP configurations may need to change the values. Refer to the documentation for additional details.
...
- `auth_ldap_1_user_flags_by_group` (Optional, Computed) Terraform object value. Retrieve users from a given group. At this time, superuser and system auditors are the only groups supported. Refer to the documentation for more detail.
...
- `auth_ldap_2_connection_options` (Optional, Computed) Terraform object value. Additional options to set for the LDAP connection. LDAP referrals are disabled by default (to prevent certain LDAP queries from hanging with AD). Option names should be strings (e.g. "OPT_REFERRALS"). Refer to https://www.python-ldap.org/doc/html/ldap.html#options for possible options and values that can be set.
...
- `auth_ldap_2_group_type_params` (Optional, Computed) Terraform object value. Key value parameters to send the chosen group type init method.
- `auth_ldap_2_organization_map` (Optional, Computed) Terraform object value. Mapping between organization admins/users and LDAP groups. This controls which users are placed into which organizations relative to their LDAP group memberships. Configuration details are available in the documentation.
...
- `auth_ldap_2_team_map` (Optional, Computed) Terraform object value. Mapping between team members (users) and LDAP groups. Configuration details are available in the documentation.
- `auth_ldap_2_user_attr_map` (Optional, Computed) Terraform object value. Mapping of LDAP user schema to API user attributes. The default setting is valid for ActiveDirectory but users with other LDAP configurations may need to change the values. Refer to the documentation for additional details.
- `auth_ldap_2_user_dn_template` (Optional, Computed) Alternative to user search, if user DNs are all of the same format. This approach is more efficient for user lookups than searching if it is usable in your organizational environment. If this setting has a value it will be used instead of AUTH_LDAP_USER_SEARCH.
- `auth_ldap_2_user_flags_by_group` (Optional, Computed) Terraform object value. Retrieve users from a given group. At this time, superuser and system auditors are the only groups supported. Refer to the documentation for more detail.
...
- `auth_ldap_3_connection_options` (Optional, Computed) Terraform object value. Additional options to set for the LDAP connection. LDAP referrals are disabled by default (to prevent certain LDAP queries from hanging with AD). Option names should be strings (e.g. "OPT_REFERRALS"). Refer to https://www.python-ldap.org/doc/html/ldap.html#options for possible options and values that can be set.
...
- `auth_ldap_3_group_type_params` (Optional, Computed) Terraform object value. Key value parameters to send the chosen group type init method.
- `auth_ldap_3_organization_map` (Optional, Computed) Terraform object value. Mapping between organization admins/users and LDAP groups. This controls which users are placed into which organizations relative to their LDAP group memberships. Configuration details are available in the documentation.
...
- `auth_ldap_3_team_map` (Optional, Computed) Terraform object value. Mapping between team members (users) and LDAP groups. Configuration details are available in the documentation.
- `auth_ldap_3_user_attr_map` (Optional, Computed) Terraform object value. Mapping of LDAP user schema to API user attributes. The default setting is valid for ActiveDirectory but users with other LDAP configurations may need to change the values. Refer to the documentation for additional details.
- `auth_ldap_3_user_dn_template` (Optional, Computed) Alternative to user search, if user DNs are all of the same format. This approach is more efficient for user lookups than searching if it is usable in your organizational environment. If this setting has a value it will be used instead of AUTH_LDAP_USER_SEARCH.
- `auth_ldap_3_user_flags_by_group` (Optional, Computed) Terraform object value. Retrieve users from a given group. At this time, superuser and system auditors are the only groups supported. Refer to the documentation for more detail.
...
- `auth_ldap_4_connection_options` (Optional, Computed) Terraform object value. Additional options to set for the LDAP connection. LDAP referrals are disabled by default (to prevent certain LDAP queries from hanging with AD). Option names should be strings (e.g. "OPT_REFERRALS"). Refer to https://www.python-ldap.org/doc/html/ldap.html#options for possible options and values that can be set.
...
- `auth_ldap_4_group_type_params` (Optional, Computed) Terraform object value. Key value parameters to send the chosen group type init method.
- `auth_ldap_4_organization_map` (Optional, Computed) Terraform object value. Mapping between organization admins/users and LDAP groups. This controls which users are placed into which organizations relative to their LDAP group memberships. Configuration details are available in the documentation.
...
- `auth_ldap_4_team_map` (Optional, Computed) Terraform object value. Mapping between team members (users) and LDAP groups. Configuration details are available in the documentation.
- `auth_ldap_4_user_attr_map` (Optional, Computed) Terraform object value. Mapping of LDAP user schema to API user attributes. The default setting is valid for ActiveDirectory but users with other LDAP configurations may need to change the values. Refer to the documentation for additional details.
- `auth_ldap_4_user_dn_template` (Optional, Computed) Alternative to user search, if user DNs are all of the same format. This approach is more efficient for user lookups than searching if it is usable in your organizational environment. If this setting has a value it will be used instead of AUTH_LDAP_USER_SEARCH.
- `auth_ldap_4_user_flags_by_group` (Optional, Computed) Terraform object value. Retrieve users from a given group. At this time, superuser and system auditors are the only groups supported. Refer to the documentation for more detail.
...
- `auth_ldap_5_connection_options` (Optional, Computed) Terraform object value. Additional options to set for the LDAP connection. LDAP referrals are disabled by default (to prevent certain LDAP queries from hanging with AD). Option names should be strings (e.g. "OPT_REFERRALS"). Refer to https://www.python-ldap.org/doc/html/ldap.html#options for possible options and values that can be set.
...
- `auth_ldap_5_group_type_params` (Optional, Computed) Terraform object value. Key value parameters to send the chosen group type init method.
- `auth_ldap_5_organization_map` (Optional, Computed) Terraform object value. Mapping between organization admins/users and LDAP groups. This controls which users are placed into which organizations relative to their LDAP group memberships. Configuration details are available in the documentation.
...
- `auth_ldap_5_team_map` (Optional, Computed) Terraform object value. Mapping between team members (users) and LDAP groups. Configuration details are available in the documentation.
- `auth_ldap_5_user_attr_map` (Optional, Computed) Terraform object value. Mapping of LDAP user schema to API user attributes. The default setting is valid for ActiveDirectory but users with other LDAP configurations may need to change the values. Refer to the documentation for additional details.
- `auth_ldap_5_user_dn_template` (Optional, Computed) Alternative to user search, if user DNs are all of the same format. This approach is more efficient for user lookups than searching if it is usable in your organizational environment. If this setting has a value it will be used instead of AUTH_LDAP_USER_SEARCH.
- `auth_ldap_5_user_flags_by_group` (Optional, Computed) Terraform object value. Retrieve users from a given group. At this time, superuser and system auditors are the only groups supported. Refer to the documentation for more detail.
...
- `auth_ldap_connection_options` (Optional, Computed) Terraform object value. Additional options to set for the LDAP connection. LDAP referrals are disabled by default (to prevent certain LDAP queries from hanging with AD). Option names should be strings (e.g. "OPT_REFERRALS"). Refer to https://www.python-ldap.org/doc/html/ldap.html#options for possible options and values that can be set.
...
- `auth_ldap_group_type_params` (Optional, Computed) Terraform object value. Key value parameters to send the chosen group type init method.
- `auth_ldap_organization_map` (Optional, Computed) Terraform object value. Mapping between organization admins/users and LDAP groups. This controls which users are placed into which organizations relative to their LDAP group memberships. Configuration details are available in the documentation.
...
- `auth_ldap_team_map` (Optional, Computed) Terraform object value. Mapping between team members (users) and LDAP groups. Configuration details are available in the documentation.
- `auth_ldap_user_attr_map` (Optional, Computed) Terraform object value. Mapping of LDAP user schema to API user attributes. The default setting is valid for ActiveDirectory but users with other LDAP configurations may need to change the values. Refer to the documentation for additional details.
- `auth_ldap_user_dn_template` (Optional, Computed) Alternative to user search, if user DNs are all of the same format. This approach is more efficient for user lookups than searching if it is usable in your organizational environment. If this setting has a value it will be used instead of AUTH_LDAP_USER_SEARCH.
- `auth_ldap_user_flags_by_group` (Optional, Computed) Terraform object value. Retrieve users from a given group. At this time, superuser and system auditors are the only groups supported. Refer to the documentation for more detail.
...
- `awx_task_env` (Optional, Computed) Terraform object value. Additional environment variables set for playbook runs, inventory updates, project updates, and notification sending.
...
- `galaxy_task_env` (Optional, Computed) Terraform object value. Additional environment variables set for invocations of ansible-galaxy within project updates. Useful if you must use a proxy server for ansible-galaxy but not git.
...
- `license` (Optional, Computed) Terraform object value. The license controls which features and functionality are enabled. Use /api/v2/config/ to update or change the license.
...
- `named_url_formats` (Optional, Computed) Terraform object value. Read-only list of key-value pairs that shows the standard format of all available named URLs.
- `named_url_graph_nodes` (Optional, Computed) Terraform object value. Read-only list of key-value pairs that exposes named URL graph topology. Use this list to programmatically generate named URLs for resources
- `oauth2_provider` (Optional, Computed) Terraform object value. Dictionary for customizing OAuth 2 timeouts, available items are `ACCESS_TOKEN_EXPIRE_SECONDS`, the duration of access tokens in the number of seconds, `AUTHORIZATION_CODE_EXPIRE_SECONDS`, the duration of authorization codes in the number of seconds, and `REFRESH_TOKEN_EXPIRE_SECONDS`, the duration of refresh tokens, after expired access tokens, in the number of seconds.
...
- `opa_auth_custom_headers` (Optional, Computed) Terraform object value. Optional custom headers included in requests to the OPA server. Defaults to empty dictionary ({}).
...
- `social_auth_azuread_oauth2_organization_map` (Optional, Computed) Terraform object value. Mapping to organization admins/users from social auth accounts. This setting controls which users are placed into which organizations based on their username and email address. Configuration details are available in the documentation.
...
- `social_auth_azuread_oauth2_team_map` (Optional, Computed) Terraform object value. Mapping of team members (users) from social auth accounts. Configuration details are available in the documentation.
...
- `social_auth_github_enterprise_organization_map` (Optional, Computed) Terraform object value. Mapping to organization admins/users from social auth accounts. This setting controls which users are placed into which organizations based on their username and email address. Configuration details are available in the documentation.
...
- `social_auth_github_enterprise_org_organization_map` (Optional, Computed) Terraform object value. Mapping to organization admins/users from social auth accounts. This setting controls which users are placed into which organizations based on their username and email address. Configuration details are available in the documentation.
...
- `social_auth_github_enterprise_org_team_map` (Optional, Computed) Terraform object value. Mapping of team members (users) from social auth accounts. Configuration details are available in the documentation.
...
- `social_auth_github_enterprise_team_map` (Optional, Computed) Terraform object value. Mapping of team members (users) from social auth accounts. Configuration details are available in the documentation.
...
- `social_auth_github_enterprise_team_team_map` (Optional, Computed) Terraform object value. Mapping of team members (users) from social auth accounts. Configuration details are available in the documentation.
...
- `social_auth_github_organization_map` (Optional, Computed) Terraform object value. Mapping to organization admins/users from social auth accounts. This setting controls which users are placed into which organizations based on their username and email address. Configuration details are available in the documentation.
...
- `social_auth_github_org_team_map` (Optional, Computed) Terraform object value. Mapping of team members (users) from social auth accounts. Configuration details are available in the documentation.
...
- `social_auth_github_team_map` (Optional, Computed) Terraform object value. Mapping of team members (users) from social auth accounts. Configuration details are available in the documentation.
...
- `social_auth_github_team_team_map` (Optional, Computed) Terraform object value. Mapping of team members (users) from social auth accounts. Configuration details are available in the documentation.
- `social_auth_google_oauth2_auth_extra_arguments` (Optional, Computed) Terraform object value. Extra arguments for Google OAuth2 login. You can restrict it to only allow a single domain to authenticate, even if the user is logged in with multple Google accounts. Refer to the documentation for more detail.
...
- `social_auth_google_oauth2_organization_map` (Optional, Computed) Terraform object value. Mapping to organization admins/users from social auth accounts. This setting controls which users are placed into which organizations based on their username and email address. Configuration details are available in the documentation.
- `social_auth_google_oauth2_secret` (Optional, Computed, Sensitive) The OAuth2 secret from your web application.
- `social_auth_google_oauth2_team_map` (Optional, Computed) Terraform object value. Mapping of team members (users) from social auth accounts. Configuration details are available in the documentation.
...
- `social_auth_organization_map` (Optional, Computed) Terraform object value. Mapping to organization admins/users from social auth accounts. This setting controls which users are placed into which organizations based on their username and email address. Configuration details are available in the documentation.
...
- `social_auth_saml_enabled_idps` (Optional, Computed) Terraform object value. Configure the Entity ID, SSO URL and certificate for each identity provider (IdP) in use. Multiple SAML IdPs are supported. Some IdPs may provide user data using attribute names that differ from the default OIDs. Attribute names may be overridden for each IdP. Refer to the Ansible documentation for additional details and syntax.
...
- `social_auth_saml_organization_attr` (Optional, Computed) Terraform object value. Used to translate user organization membership.
- `social_auth_saml_organization_map` (Optional, Computed) Terraform object value. Mapping to organization admins/users from social auth accounts. This setting controls which users are placed into which organizations based on their username and email address. Configuration details are available in the documentation.
- `social_auth_saml_org_info` (Optional, Computed) Terraform object value. Provide the URL, display name, and the name of your app. Refer to the documentation for example syntax.
- `social_auth_saml_security_config` (Optional, Computed) Terraform object value. A dict of key value pairs that are passed to the underlying python-saml security setting https://github.com/onelogin/python-saml#settings
...
- `social_auth_saml_sp_extra` (Optional, Computed) Terraform object value. A dict of key value pairs to be passed to the underlying python-saml Service Provider configuration setting.
...
- `social_auth_saml_support_contact` (Optional, Computed) Terraform object value. Provide the name and email address of the support contact for your service provider. Refer to the documentation for example syntax.
- `social_auth_saml_team_attr` (Optional, Computed) Terraform object value. Used to translate user team membership.
- `social_auth_saml_team_map` (Optional, Computed) Terraform object value. Mapping of team members (users) from social auth accounts. Configuration details are available in the documentation.
- `social_auth_saml_technical_contact` (Optional, Computed) Terraform object value. Provide the name and email address of the technical contact for your service provider. Refer to the documentation for example syntax.
- `social_auth_saml_user_flags_by_attr` (Optional, Computed) Terraform object value. Used to map super users and system auditors from SAML.
- `social_auth_team_map` (Optional, Computed) Terraform object value. Mapping of team members (users) from social auth accounts. Configuration details are available in the documentation.
