...
- [ ] 1.2 Create or designate the **GitHub repository** that should receive the default branch and set branch protection consistent with an automated force-update of that branch
...
- [x] 2.2 Add `.gitlab-ci.yml` with `GIT_DEPTH: "0"`, a `mirror_to_github` (or similarly named) job, `rules` limiting to `$CI_COMMIT_BRANCH == $CI_DEFAULT_BRANCH`, Alpine or equivalent image with `git`, `curl`, `jq`, `openssl`, `bash`, and steps to force-push only the default branch to GitHub and delete stale `refs/merge-requests/*` / `refs/pipelines/*`
- [x] 2.3 Document operator steps in [`AGENTS.md`](../../../AGENTS.md) (short subsection: variables, GitHub App permissions, force-update implications)
