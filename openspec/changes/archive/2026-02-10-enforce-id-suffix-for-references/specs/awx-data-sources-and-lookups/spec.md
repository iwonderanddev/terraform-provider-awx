...
The provider SHALL expose data sources for managed AWX object types needed to reference existing infrastructure in Terraform configurations. Data source attributes that represent numeric AWX reference identifiers SHALL use Terraform number types that are directly consumable by resource arguments expecting the same references. Data source reference/link attributes SHALL use explicit `_id` suffix names, and unsuffixed link aliases SHALL NOT be generated. Data source `id` SHALL be `Number` for collection-created objects and `String` for detail-path keyed objects.
...
#### Scenario: Data source reference field naming is explicit
- **WHEN** a data source returns an attribute that links to another AWX object
- **THEN** the returned attribute name uses a canonical `_id` suffix
