...
For survey-spec relationship resources, the `spec` argument SHALL be a native
Terraform structured object value; JSON-string configuration for `spec` SHALL NOT
be supported.
...
- **THEN** the provider accepts canonical parent object identifier argument
  naming (for example, `job_template_id`) together with `spec` as a Terraform
